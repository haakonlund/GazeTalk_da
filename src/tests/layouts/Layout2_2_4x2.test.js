...
describe("Layout2_2_4x2", () => {
  const dwellTime = 2000;

  test("switches to layout '2+2+4x2' when the corresponding layoutTile is hovered for the dwell time", async () => {
    ...
    render(<App initialView="layouts" initialLayout="2+3+5x2" />);
    ...
    expect(metadataBefore).toHaveAttribute("data-cols", "5");


    const layoutTile = screen.getByRole("button", { name: /2\+2\+4x2/i });
  ...
  render(<App initialView="suggestions" initialLayout="2+2+4x2" />);
