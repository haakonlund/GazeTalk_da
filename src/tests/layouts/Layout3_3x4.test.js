...
describe("Layout3_3x4", () => {
  const dwellTime = 2000;

  test("switches to layout '3+3x4' when the corresponding layoutTile is hovered for the dwell time", async () => {
    ...
    const layoutTile = screen.getByRole("button", { name: /3\+3x4/i });
  ...
  test("renders the expected number of tiles in writing view", async () => {
    ...
    render(<App initialView="writing" initialLayout="3+3x4" />);
    await waitFor(() => {
  ...
  render(<App initialView="suggestions" initialLayout="3+3x4" />);
