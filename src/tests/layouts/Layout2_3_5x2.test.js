...
describe("Layout2_3_5x2", () => {
  const dwellTime = 2000;


  test("switches to layout '2+3+5x2' when the corresponding layoutTile is hovered for the dwell time", async () => {
    ...
    const layoutTile = screen.getByRole("button", { name: /2\+3\+5x2/i });
    ...
    await waitFor(() => {
      ...
      expect(metadataAfter.getAttribute("data-rows")).toBe("3");
  ...
  test("renders the expected number of tiles in writing view", async () => {
    ...
    render(
    <UserBehaviourTestProvidor>
     <App initialView="writing" initialLayout="2+3+5x2" unitTesting={true} />
    </UserBehaviourTestProvidor>
  );
    await waitFor(() => {
  ...
  render(
    <UserBehaviourTestProvidor>
      <App initialView="suggestions" initialLayout="2+3+5x2" unitTesting={true} />
    </UserBehaviourTestProvidor>
  );

  await waitFor(() => {
      const tileElements = document.querySelectorAll(".tile");
      expect(tileElements.length).toBe(14); // 14 since TextAreaTile = 2 tiles wide.
    });
  });
});
